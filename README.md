...
## Incentive
Often times, managing waste can be overwhelming and done incorrectly. At the recycling level, nearly 76% of recyclables are lost at the household level, and even those which are put into the recycling bin can be wrongly classified. As these wrongly-sorted articles of trash add up, the recycling process loses its efficiency, inflating unnecessary costs to over $3.5 billion yearly for the US. As a direct result, recycling becomes more of a burden than a tool. Human error is exacerbating what recycling aims to fix: bettering the environment. Instead of looking towards refining the system, we should look to fix the root of the cause, our inaccuracies, by removing ourselves from the equation. AI is better than humans probably so like make a trash sorter ig 🤷‍♂️.
